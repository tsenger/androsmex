...
public class FakeTagsActivity extends ListActivity {
    ...
    public static NdefRecord newTextRecord(String text, Locale locale, boolean encodeInUtf8) {
        ...
        final byte[] langBytes = locale.getLanguage().getBytes(Charset.defaultCharset());
        final Charset utfEncoding = Charset.forName("UTF-16");
        final byte[] textBytes = text.getBytes(utfEncoding);
        final int utfBit = encodeInUtf8 ? 0 : (1 << 7);
        final char status = (char) (utfBit + langBytes.length);
        final byte[] data = new byte[] {(byte) status};
        //Byte.
    ...
    public static NdefRecord newMimeRecord(String type, byte[] data) {
        ...
        final byte[] typeBytes = type.getBytes(Charset.defaultCharset());
    ...
    public void onCreate(Bundle savedState) {
        ...
        adapter.add(
                new TagDescription("Extra Tag", MockNdefMessages.ENGLISH_PLAIN_TEXT));
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        intent.putExtra(NfcAdapter.EXTRA_TAG, MockNdefMessages.ENGLISH_PLAIN_TEXT);
